class ModeListLinesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @mode_list_line.save
        ...
        format.json { render :show, status: :created, location: [@mode_list, @mode_list_line] }
      else
  ...
  def update
    respond_to do |format|
      if @mode_list_line.update(mode_list_line_params)
        ...
        format.json { render :show, status: :ok, location: [@mode_list, @mode_list_line] }
      else
  ...
  private
    def set_inheritenced_vars
      @mode_list = ModeList.find(params[:mode_list_id])
    end
